...
def keys_pressed(d_o_e=None, d_w=0):
    #root = tkinter.Tk()  # Create a root window
    global _root_window
    _root_window.update()  # Update the root window to process events
    _root_window.dooneevent(d_w)  # Process one event without waiting

    if _got_release:
        _root_window.dooneevent(d_w)  # Process another event if a release event occurred
